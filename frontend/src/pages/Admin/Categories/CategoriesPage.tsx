...
interface Category { id: number; name: string; slug: string; description: string; image_url: string; subcategory_count: number; }
interface Subcategory { id: number; name: string; slug: string; description: string; }
...
export default function CategoriesPage() {
  ...
  // Modal state
  const [modal, setModal] = useState<{ type: 'category' | 'subcategory'; editing: any; categoryId?: number } | null>(null);
  ...
  const openModal = (type: 'category' | 'subcategory', editing: any = null, categoryId?: number) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modal?.type === 'category') {
        ...
        if (modal?.editing) {
          const res = await api.put(`/categories/subcategories/${modal.editing.id}`, form);
          setSubcategories((prev) => ({
            ...prev,
            [modal.categoryId!]: prev[modal.categoryId!].map((s) => s.id === modal.editing.id ? res.data.subcategory : s),
          }));
          toast.success('Subcategory updated');
        } else {
          const res = await api.post(`/categories/${modal!.categoryId}/subcategories`, form);
          setSubcategories((prev) => ({
            ...prev,
            [modal!.categoryId!]: [...(prev[modal!.categoryId!] || []), res.data.subcategory],
          }));
  ...
  const handleDeleteSubcategory = async (catId: number, subId: number, name: string) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-5">
              {modal.editing ? 'Edit' : 'Add'} {modal.type === 'category' ? 'Category' : 'Subcategory'}
            </h2>
