...
export default function CheckoutPage() {
  ...
  const [address, setAddress] = useState<Address>({
    name: user?.name || '', line1: '', line2: '', city: '', state: '', zip: '', country: 'US',
  });
  ...
  const field = (label: string, key: keyof Address, props?: React.InputHTMLAttributes<HTMLInputElement>) => (
    <div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="card space-y-4">
              ...
              {field('Address Line 1', 'line1', { required: true, placeholder: '123 Main St' })}
            ...
            <div className="card space-y-4">
              ...
              <div className="flex items-center gap-3 p-3 border border-gray-300 rounded-lg bg-gray-50">
                <input type="radio" id="cod" name="payment" defaultChecked readOnly className="text-blue-600" />
                <label htmlFor="cod" className="text-sm text-gray-700 font-medium">Cash on Delivery</label>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="card sticky top-24">
              ...
              <div className="space-y-2 mb-4">
                {items.map((item) => (
                  <div key={item.product_id} className="flex gap-3">
                    ...
                    <p className="text-sm font-medium text-gray-900">${(item.effective_price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
              ...
              <button type="submit" className="btn-primary w-full mt-5 flex items-center justify-center gap-2" disabled={loading}>
                <Lock size={14} />
